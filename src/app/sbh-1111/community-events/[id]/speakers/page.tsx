...
export default async function EventSpeakersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href="/sbh-1111/community-events" className="text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button asChild>
          <Link href={`/sbh-1111/community-events/${id}/speakers/new`}>New Speaker</Link>
        </Button>
      </div>
      <DataTable
        rows={speakers}
        columns={eventSpeakerColumns}
        editHrefBase={`/sbh-1111/community-events/${id}/speakers`}
