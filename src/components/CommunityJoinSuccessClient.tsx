...
export function CommunityJoinSuccessClient({
  orderId,
  email,
  siteSettings,
  tier,
  ...
  const joinHref = `/community/join/${tier}`;
  const preferencesHref = `/community/digest/preferences${email ? `?email=${encodeURIComponent(email)}` : ""}`;
          ...
          <div className="relative mx-auto max-w-lg text-center">
            ...
            {status === "PAID" && (
              <>
                ...
                <Button asChild size="lg" variant="outline" className="mt-4 border-white/15 bg-transparent text-white hover:bg-white/10">
                  <a href={preferencesHref}>Choose your daily brief topics →</a>
                </Button>
              </>
            )}
            ...
            {status === "not-found" && (
              <>
                ...
                <p className="mt-3 text-white/50">
                  ...
                  <a href={joinHref} className="text-accent underline">head back to the membership page</a>.
                </p>
              </>
            )}
          </div>
